...
Carried as an httpOnly `SameSite=Strict` cookie, and accepted as
`Authorization: Bearer` for non-browser callers. Accepting the header does not
weaken CSRF: a cross-origin page can cause a cookie to be sent but cannot set a
header. Cookie-authenticated writes additionally check `Origin`, because
SameSite is enforced by the client and a client that does not enforce it leaves
the cookie exposed.
