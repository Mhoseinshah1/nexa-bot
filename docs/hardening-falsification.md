...
| H-08 | Every freshness-bearing loop is NAMED in the worker's health check | delete the `throttle-sweeper` row from `main.worker.ts`                  | `worker-health-coverage.test.ts` › names every worker-started loop in the worker health check | KILLED |
