...
| W06  | (bad aim — see above) the conflict TARGET names the reference index        | `.onConflictDoNothing({ target })` → `.onConflictDoNothing()` | none                                                                                     | SURVIVED |
| W06b | A movement is idempotent at the unique index, not in a process             | the whole `.onConflictDoNothing(…)` removed                   | `wallet.test.ts` › moves money ONCE for a repeated reference / when two appends race     | KILLED   |
| W07  | The keyset cursor is PostgreSQL microsecond text, not a `Date`             | `last.createdAtText` → `last.createdAt.toISOString()`         | `wallet.test.ts` › pages over MICROSECONDS, so two entries inside one millisecond …      | KILLED   |
| W08  | An amount is greater than zero and within the ceiling, at the SERVICE      | `this.assertLedgerAmount(input.amountMinor)` removed          | `wallet.test.ts` › refuses an amount of zero or past the ceiling, as a refusal not a 500 | KILLED   |
...
| W11  | The ledger reason is derived from the direction, never named by the caller | `CREDIT: 'ADMIN_CREDIT'` → `CREDIT: 'PURCHASE'`               | `wallet.test.ts` › credits once for a repeated command, and commits the audit and event  | KILLED   |
...
| W16  | The domain event commits with the entry                                    | `eventType: 'WalletEntryRecorded'` → `'CustomerRegistered'`   | `wallet.test.ts` › credits once for a repeated command, and commits the audit and event  | KILLED   |

Sixteen mutations run, fifteen rules covered, one bad aim recorded as such.
