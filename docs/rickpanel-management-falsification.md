...
| RM-02 | a 5xx on the allowance write is retryable, not a refusal           | the adapter's non-2xx answer to the allowance `PUT` → `PROVIDER_REFUSED` | `rickpanel-management.test.ts` › verifies a renewal that was applied and whose answer was lost, and refunds nothing (WP15 G2)                                                                                                                                                                                                                           | KILLED |
...
| RM-04 | the expiry sent is the stored absolute target                      | the `expire` sent is the target plus one day                             | `rickpanel-management.test.ts` › renews: charged once, applied once, and Nexa and the panel agree; `rickpanel-management.test.ts` › adds traffic without touching the expiry, and adds time without touching the traffic; `rickpanel-management.test.ts` › verifies a renewal that was applied and whose answer was lost, and refunds nothing (WP15 G2) | KILLED |
...
**WP15 re-verification.** RM-02 and RM-04 cited the lost-answer renewal case by its old
name. WP15 G2 made that case verify by a READ instead of replaying the PUT; both
mutations were re-run against the rewritten test and killed it (`docs/wp15-falsification.md`).
