...
| WP9-27 | a redelivered invite is answered again, not refused as in flight               | the `replay === null` guard around `rememberOnce` in `invite` forced true     | `referrals.test.ts` › answers a redelivered invite again from the same key, rather than refusing it as in flight                                                                                                                                                               | KILLED |
| WP9-28 | a `/start` update remembered by the previous release replays after the upgrade | the start payload put back into `resolveFromUpdate`'s request hash            | `referrals.test.ts` › replays a /start update a previous release remembered without its payload, instead of refusing it as a mismatch                                                                                                                                          | KILLED |
| WP9-29 | the invite link carries the bot's name as Telegram reports it now              | the link built from the stored `bot.username`                                 | `referrals.test.ts` › builds the link from the name Telegram reports now, not the one the bootstrap stored                                                                                                                                                                     | KILLED |
| WP9-30 | no link is offered when Telegram cannot name the bot                           | the `username === null` → `UNAVAILABLE` answer removed                        | `referrals.test.ts` › offers no link, and records no code, when Telegram cannot say what the bot is called                                                                                                                                                                     | KILLED |

WP9-27 to WP9-30 are the fixes for the Codex review of PR #68. They were mutated in the
implementation worktree, one at a time, each restored with `cp` from a saved copy before the
next, and `git diff` was read afterwards to confirm the tree held only the fixes.
