...
| Product name, volume, duration | The order snapshot: `line_title`, `line_traffic_bytes` and `line_duration_days` (`schema.ts:3013-3015`). Rendered through the `BYTES` and `DURATION_DAYS` placeholder types.                                                          | Yes, for order payments. A top-up shows a dash.                                                                                                               |
