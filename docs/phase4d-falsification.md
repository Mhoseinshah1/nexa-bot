...
| F4D-15 | An UNKNOWN provider outcome moves the service to `UNRECONCILED`      | `serviceEventFor` UNKNOWN → `null` instead of `PROVISION_LOST_TRACK`                       | `provisioning-delivery.test.ts` › creates no second account, and adopts none it cannot prove, when a create is cut off after the panel stored it | KILLED   |
| F4D-16 | The delivery sweep takes ACTIVE services only                        | `eq(services.state, 'ACTIVE')` removed from `claimDeliveryDue`                             | `provisioning-delivery.test.ts` › creates no second account, and adopts none it cannot prove, when a create is cut off after the panel stored it | KILLED   |
...
| F4D-21 | A reconcile is planned for an unresolved unknown outcome             | `await this.planReconciles(scope, now);` removed                                           | `provisioning-delivery.test.ts` › creates no second account, and adopts none it cannot prove, when a create is cut off after the panel stored it | KILLED   |
...
**WP15 re-verification.** F4D-15, F4D-16 and F4D-21 cited the lost-reply test by its
old name. The owner's WP15 decisions (G6, G7) changed what that case ends in — the
create is UNKNOWN and is never adopted without provenance — and the test was rewritten
and renamed. Each of the three mutations was re-run against the rewritten test, serially,
and each killed it (`docs/wp15-falsification.md`).
