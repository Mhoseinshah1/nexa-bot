...
| Reports             | —                                                             | WP12 (PR #76) reports reseller orders, sales, services and credit in use without margin or cost. The review of this package found its credit figures read the balance in the LIMIT's currency and ignored a tier limit; they now come from the same `creditAllowanceOf` / `creditFigures` derivation as the credit card. |
...
- **Source:** the snapshot, never live tier settings (R9). Margin is not selected (§3) —
  but list amount and cost are, and a CHECK makes margin their difference, so this is not
  a secrecy boundary. It does not need to be one: `orders.view` already reads
  `marginAmount` on each order's pricing card (review of this package).
...
- **The confirmations are a courtesy of the page, not a server rule** (review of this
  package). Nothing is lost when one is skipped — R8 leaves the debt where it is — but the
  page skips it silently when the credit read has not loaded or failed, when the operator
  holds `resellers.edit` without `users.view`, when the tier list has not loaded, and for a
  direct API call. Making the server require an acknowledgement would be a new rule; it
  is recorded as `OQ-WP14-01` for the owner, not added here.
- **The credit card's figures are three reads, not one snapshot** — the listing, the
  setting and the balance. A limit change or purchase landing between them can show a
  limit and a balance from different instants. It is display-only; settlement decides
