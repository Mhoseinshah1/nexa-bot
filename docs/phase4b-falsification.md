...
| M16 | A catalogue button carries its price, through the shared money renderer          | drop the `formatMoney` half of the label                     | `telegram-order-flow.test.ts` › lists every sellable product in the category as a button carrying its id, and nothing else | KILLED |
| M17 | A reply with buttons sends `reply_markup`                                        | the `buttons.length > 0` guard → `if (false)`                | `telegram-order-flow.test.ts` › lists every sellable product in the category as a button carrying its id, and nothing else | KILLED |
...
## M16 and M17 after WP5

WP5 made the catalogue two steps, category then product, and renamed the test M16 and
M17 cite to _lists every sellable product in the category as a button carrying its id,
and nothing else_. Both rows were re-run against the renamed test rather than re-pointed
on trust: dropping the money half of the label kills it (1 failed | 27 passed), and the
`reply_markup` guard → `if (false)` kills it along with seven other catalogue cases.
