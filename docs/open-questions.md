...
| `OQ-RP-04`                  | How long does RickPanel node propagation take?                                                     | Timed reads after a create. Until measured, a RECONCILE after "accepted, not readable yet" can read ABSENT too early (WP15 audit §3, G3).                    |
| `OQ-RP-05`                  | After an UNKNOWN create, can a found account be proved to be ours?                                 | Durable per-operation provenance — a schema change. Shared with Marzban and 3X-UI, whose 409 / duplicate-email path adopts.                                  |
...
| `OQ-RP-09`                  | A reset after the request was written is UNREACHABLE (safe to replay). Safe for a create?          | See `OQ-PROV-01` above.                                                                                                                                      |
...
| `OQ-WP15-RICK-USED-TRAFFIC` | Does RickPanel's user record carry `used_traffic`?                                                 | One read of a user that has carried traffic. `lookupUser` refuses a found record without it (MALFORMED), so every RickPanel RECONCILE depends on the answer. |
| `OQ-WP15-RICK-NUMBERS`      | Does RickPanel echo `expire` and `data_limit` as numbers? The document types them as strings.      | One PUT read back. If strings, `appliedPlan`'s strict comparison would call every applied renewal MALFORMED and refund it (WP15 audit G2).                   |
