...
| Id                       | Question                                                                                                                                                                                         | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `UNK-XUI-010`            | Which authentication mechanism does the Sanaei / 3x-ui panel this installation talks to actually use, and at which version?                                                                      | **Closed in 3B, row left for history (WP15 ledger correction).** `packages/contracts/src/provider.ts` records it CLOSED: the upstream v3.7.0 source settles the auth modes and the fake reproduces them, and the real v3.7.0 acceptance (`docs/real-panel-acceptance.md`) ran the create path. What follows is the text as it stood before: The descriptor records `USERNAME_PASSWORD` because that is 3X-UI's documented login, and says so explicitly — it is **not** a corpus finding. The corpus marks the panel's API `NOT_EXPOSED`, which is not the same as absent. 3C must confirm against a real panel or a fixture derived from one. |
| `Q-MARZBAN-PROBE`        | The Marzban adapter's endpoints (`POST api/admin/token`, `GET api/system`) come from Marzban's documented API, not from the research corpus, and have not been run against a real panel.         | **Closed by the real Gozargah/Marzban v0.8.4 acceptance** (`docs/real-panel-acceptance.md`, WP15 ledger correction). Before it: Unit tests drive the adapter against a scripted client, so they prove the normalization and not the endpoints. First real-panel contact will confirm or correct them.                                                                                                                                                                                                                                                                                                                                          |
...
**WP15 correction (`docs/wp15-provider-hardening-audit.md`).** The containment argument
above is stale on two counts, and the question is therefore more open than it reads:

- The provider username is no longer derived from the service id. It is the per-panel
  policy of `docs/phase6c-username-falsification.md`: a reserved CUSTOM or RANDOM name.
  The replay still addresses the SAME name, which is the part of the argument that
  holds.
- RickPanel answers a taken name with **409 → `PROVIDER_REFUSED`**, which is terminal and
  refunds (Codex C2 on PR #58 removed adoption on 409 so a foreign account is never
  delivered). So for RickPanel a create whose connection is torn down after the panel
  committed it reads UNREACHABLE → replayed → 409 → **refund, with the account left on
  the panel**. Marzban and 3X-UI still reach reconciliation as described.

**WP15 G6/G7 (owner decision) supersedes the paragraph below.** A write whose connection
fails after it may have been sent is now `TIMEOUT` + `CONNECTION_LOST_AFTER_SEND`, never
replayed; the create goes UNKNOWN, and a found account is adopted ONLY with provenance
(`create_accepted_at`). A lost answer is not provenance, so such a service waits for an
operator (`FOUND_WITHOUT_PROVENANCE`) rather than being replayed, refunded or adopted.

WP15 closed the half of this that is in the adapter: a READ-BACK failure after an
accepted create is now UNKNOWN (H1), and `SafeHttpClient` never retries a write itself
(H4). The half at the socket — telling a reset before the request was flushed from one
after it — is still open, is `OQ-RP-09`, and is unchanged.

## OQ-WP15 — provider questions a live panel must settle

Collected here so the ledger holds every provider question (they previously lived only
in the RickPanel audits). None is resolved by WP15; each needs the named measurement.

| Id                          | Question                                                                                           | What settles it                                                                                                                                               |
| --------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OQ-RP-01`                  | Subscription field names on RickPanel's user record.                                               | Answered for `subscription_url` by the owner's direct calls (`docs/rickpanel-create-hotfix.md`); the adapter still accepts the other shapes and says so.      |
| `OQ-RP-02`                  | Does RickPanel's create accept `status`?                                                           | A create on a disposable user with `status` set, read back.                                                                                                   |
| `OQ-RP-03`                  | What does RickPanel's create return?                                                               | Not depended on: the adapter reads the user back either way.                                                                                                  |
...
| `OQ-WP15-RICK-USED-TRAFFIC` | Does RickPanel's user record carry `used_traffic`?                                                 | Still a live question. WP15 G5: a record without it is `USAGE_FIELD_MISSING`, retried, never zero — if RickPanel never sends it, reconcile needs an operator. |
| `OQ-WP15-RICK-NUMBERS`      | Does RickPanel echo `expire` and `data_limit` as numbers? The document types them as strings.      | **Code-side closed:** canonical numeric strings are read exactly like numbers (`provider-numbers.ts`). What a real panel sends is still unmeasured.           |
| `OQ-WP15-ADOPT`             | An operator who KNOWS a found account is theirs has no "adopt" action (WP15 G7).                   | Product decision. Today's exit: operator TERMINATE (Nexa-side, no DELETE) and the operator refund. Not built without an owner instruction.                    |
| `OQ-WP15-ALLOWANCE-EXIT`    | An `ALLOWANCE_UNVERIFIED` renewal has no operator action to re-verify or settle it (WP15 G2).      | Product decision: an operator re-arm of the three reads, or a decided outcome. Until then the row is visible and holds the order; no money moves.             |
