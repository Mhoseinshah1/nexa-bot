...
| UX10 | add-traffic applies once on replay                | `provisioning.service.ts` `planCommercialAction`: the operation id suffixed with the clock, so a replay plans a second operation           | SURVIVED (WP15 re-run) | `tests/integration/rickpanel-management.test.ts` › adds traffic without touching the expiry, and adds time without touching the traffic                                                                                                                                   |
| UX11 | renewal applies once on replay                    | same line as UX10                                                                                                                          | SURVIVED (WP15 re-run) | `tests/integration/rickpanel-management.test.ts` › renews: charged once, applied once, and Nexa and the panel agree                                                                                                                                                       |
...
**WP15 re-run of UX10 and UX11.** Re-running the shared mutation (the commercial
operation id suffixed with the clock) serially against `rickpanel-management.test.ts`
and `service-management.test.ts` kills no test. The reason is structural, not a lost
test: a second plan for the same service is refused by
`provisioning_operations_open_commercial_key` and the fulfillability check before any
id is compared, and a settlement replay is absorbed by the payment's idempotency key.
So the derived id is a second guard nothing currently exercises alone. The rows are
marked SURVIVED rather than left claiming a kill; UX10's second citation named a test
WP15 G2 rewrote. Whether this needs its own test is recorded, not decided here.
