...
The pass was **bounded on purpose**. Thirteen mutations, each aimed at one rule
the owner named, driven by `/tmp/falsify-4a.py` — which prints the applied diff
...
Thirteen of thirteen killed — **after two of them were fixed, and the fixing is
the finding.**
