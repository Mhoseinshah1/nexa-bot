...
| F4E-32  | A customer is offered no end button at all (WP15 G1, which replaced "ending a service takes two taps") | the `t:` end button drawn on the service card again                       | `service-management.test.ts` › offers pause on an active service, and NO end button at all (WP15 G1)             | KILLED |
...
| F4E-35  | A customer's request checks the state the action is legal from                                         | `if (!legalFrom.includes(service.state))` → `if (false && …)`             | `service-management.test.ts` › offers nothing further once an OPERATOR has ended the service                     | KILLED |
...
**WP15 re-verification.** WP15 G1 removed customer termination, so F4E-32's rule is now
"no end button is drawn" and F4E-35 is carried by a stale pause on an operator-ended
service. Both mutations were re-run against the rewritten tests and killed them
