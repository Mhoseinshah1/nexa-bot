...
import {
  ...
  REFERRAL_GIFT_CALLBACK_DATA,
  SERVICE_NOTE_CALLBACK_PREFIX,
  SERVICE_RENEW_QUOTE_CALLBACK_PREFIX,
  TOPUP_CLOSE_CALLBACK_PREFIX,
  encodeIdPair,
...
describe('a callback prefix decides what happens, so no prefix may shadow another', () => {
  ...
  /** The customer UX completion's routes that carry no uuid: a page, an enum, a pair of them, or nothing. */
  const OTHER_PREFIXES: Readonly<Record<string, string>> = {
    MAIN_MENU: MAIN_MENU_CALLBACK_DATA,
    TUTORIAL: TUTORIAL_CALLBACK_DATA,
    TUTORIAL_PLATFORM: TUTORIAL_PLATFORM_CALLBACK_PREFIX,
    SUPPORT: SUPPORT_CALLBACK_DATA,
    TOPUP_ROUTE: TOPUP_ROUTE_CALLBACK_PREFIX,
